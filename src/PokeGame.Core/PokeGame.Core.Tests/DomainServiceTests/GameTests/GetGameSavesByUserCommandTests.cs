...
public sealed class GetGameSavesByUserCommandTests
{
    ...
    public async Task ExecuteAsync_Should_Return_GameSaves_When_Repository_Returns_Data()
    {
        ...
        _mockGameSaveRepository
            .Setup(x => x.GetManyAsync<Guid>((Guid)user.Id!, nameof(GameSaveEntity.UserId), CancellationToken.None, nameof(GameSaveEntity.GameSaveData)))
        ...
        _mockGameSaveRepository.Verify(x => x.GetManyAsync<Guid>((Guid)user.Id!, nameof(GameSaveEntity.UserId), CancellationToken.None, nameof(GameSaveEntity.GameSaveData)), Times.Once);
    }
