...
internal sealed class InGrassRandomPokemonEncounterCommand: IDomainCommand<(string SceneName, string ConnectionId, Schemas.Game.User CurrentUser), DomainCommandResult<WildPokemon?>>
 {
         ...
         CancellationToken cancelationToken = default)
     {
         ...
         if(!GameConstants.PokemonGameRules
                .SceneWildPokemonRange
                .TryGetValue(input.SceneName, out var sceneWildPokemonRange))
         {
             throw new PokeGameApiUserException(HttpStatusCode.BadRequest,
                 "Invalid pokegame scene name");
         }
         
         var randomPokemonId = _pokeGameRuleHelperService
             .GetRandomPokemonNumberFromPokedexRangeWithRandomEncounterIncluded(sceneWildPokemonRange.PokedexRange);
         ...
         var (foundPokemon, foundPokemonSpecies) = await GetPokemonFromApi((int)randomPokemonId!, 
             cancelationToken);
         ...
         CancellationToken cancellationToken)
     {
         ...
         return (await pokemonApiJob, await  pokemonSpeciesJob);
     }
