...
internal sealed class EndGameSessionCommand: IDomainCommand<
        string,
        DomainCommandResult
    >
{
    ...
    public EndGameSessionCommand(IGameSessionRepository gameSessionRepository,
    ...
    public async Task<DomainCommandResult> ExecuteAsync(string connectionId, CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("About to end game session for connection Id: {ConnectionId}",  connectionId);
        ...
        {
            throw new PokeGameApiUserException(HttpStatusCode.BadRequest, "Game session with that id doesn't exist");
        }
        var endedResult = await EntityFrameworkUtils
            .TryDbOperation(() => _gameSessionRepository.EndGameSession(foundGameSession.Data), _logger)
