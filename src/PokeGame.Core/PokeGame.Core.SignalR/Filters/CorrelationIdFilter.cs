...
internal sealed class CorrelationIdFilter : IHubFilter
{
    ...
    )
    {
        ...
        using (
            _logger.BeginScope(
                ...
                {
                    [ApiConstants.CorrelationIdHeader] = addedCorrelationId,
                }
            )
        )
        {
    ...
    private string AddCorrelationIdToHttpContext(HttpContext? context)
    {
        ...
        {
            _logger.LogWarning(
                "Signal R filter failed to add correlationId to response headers..."
            );
        }
        else
        {
            _logger.LogInformation(
                "Signal R filter successfully added correlationId: {CorrelationId} to response headers...",
                correlationId
            );
        }
