...
public static class HttpContextExtensions
{
    ...
    public static void TryAddToItems<T>(
    ...
    )
    {
        ...
        {
            logger?.LogDebug("Added item: {@Item} with key: {Key} to request items", item, itemKey);
        }
        else
        {
            logger?.LogWarning(
                "Failed to add item: {@Item} with key: {Key} to request items",
                item,
                itemKey
            );
        }
